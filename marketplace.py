...
def add_orders_to_supplie_by_id(
        token: str, supplie_id: str, orders_ids: List[str]):
    headers = {
        'Authorization': token,
    }
...
def edit_blank_pdf(barcode_info, name):
    ...
    for params in (count, name_of_host, name, size,
...
def create_pdf_stickers_by_barcodes(token, name, barcodes_and_ids):
    results_files = []
    results_files = create_and_merge_pdf_by_barcodes_and_ids(
        token, name, barcodes_and_ids)
    merger = PdfFileMerger()
...
def get_all_orders(token, status=0, date_end=get_now_time(),
...
def create_db_for_checking(token, barcodes):
    ...
    sheet.conditional_formatting.add(
        'C1:C3000',
        FormulaRule(
            formula=['IF(AND(C1=B1, C1<>""),True,False)'],
            stopIfTrue=True,
            fill=green_fill))
    ...
    sheet.conditional_formatting.add(
        'B1:B3000',
        FormulaRule(
            formula=['IF(B1="Ошибка",True,False)'],
            stopIfTrue=True,
            fill=red_fill))
